...
当前版本见 [VERSION](VERSION) 和 [GitHub Releases](https://github.com/zcq19991029/kegai-skills/releases)；版本号时间使用北京时间。
...
Skill 会按顺序请你确认：**写文件前**核对课程、班级、排课口径、课次、时长和保存位置；**写出课次分配 Markdown 后**检查内容是否需要调整。你可以直接改 Markdown，或指出“第 X 次课改为……”。Skill 更新并回复“已更新”后，会重新读取最新文件，再问你要生成多少次、指定第几次，或第几次至第几次课的教学设计。确定本批范围后才生成对应文件；同一批已确认的信息无需逐份重复确认。

第一次课可用课程说明和生活案例导入，无须虚构本课程旧知。PPT按进度选用：一份PPT可能分多次讲，一次课也可能覆盖几个小节。修改成品后只保留一个最新文件，不留多个带版本后缀的副本。

若要给已有教学设计文件名加上上课日期，请提供第1教学周的日期锚点（例如“第1次课是2026年9月1日周二”）和已知的补课通知。Skill 会按**各班**课表计算日期；同一天连堂的两个80分钟文件日期相同。文件名默认保留原排日期，已确认的补课写作 `2026年10月06日（实际2026年09月20日）`。学校尚未通知的调课先不推测。只要求改文件名时，Word正文和课次分配Markdown均不改动，并检查改名前后文件内容一致。

课程资料、设备条件或教师安排发生变化时，可直接说“更新第X次至第Y次课，两个班同步”。Skill 会先更新本地课次分配Markdown，再逐份检查目标、演示、任务、验收、评价和作业，修订原有同名Word文件。例如将后续课次的仿真改为Keil与实际开发板实训时，验收证据也应改为开发板运行记录；两班同序号内容一致，但各自上课日与连堂衔接仍按课表处理。

教学组织也按你的实际课堂来定。若明确“一人一机、每人独立完成”，材料会写个人操作、个人提交及教师巡视收取和评价，不套用分组角色或组间互评。AI工具只有与本课任务直接相关且确实安排使用时才写入课堂活动；不会为凑清单加入空泛句子。

若规定“每节课前先签到、再交手机，课中禁用手机”，请在指令中说明。Skill 会把签到窗口安排在上课前，课堂测验和提交改为机房电脑操作，开发板成果用测试记录与教师现场验收。若课程不使用AI，直接说明“本课程不安排AI”，相关工具和活动会从整批材料中去除。
